...
def train(
        # model/data params
        load_8bit: bool = True,  # for 7B, we can load as fp16/native
        base_model: str = "",  # the only required argument
        data_path: str = "",
        data_path2: str = "",  # inputs and outputs only
    ...
    def preprocess_function(d):
        return {
            "instruction": d.get("instruction", "Fix grammar:"),
            "input": d.get("input", None),
            "output": d.get("output", None),
        }

    # support multi json files
    file_paths = [data_path, data_path2]
    for f in file_paths:
        if not os.path.exists(f):
            print(f + " file/path does not exists")
            sys.exit(1)

    data = load_dataset("json", data_files=file_paths)
